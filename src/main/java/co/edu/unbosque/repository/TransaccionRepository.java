...
public interface TransaccionRepository extends JpaRepository<Transaccion, Integer> {
        ...
        List<Object[]> findByInversionistaIdAndEstado(@Param("inversionistaId") Integer inversionistaId,
        ...
        List<Object[]> findByInversionistaIdAndEstadoAndTipo(@Param("inversionistaId") Integer inversionistaId,
                        @Param("estado") Boolean estado, @Param("tipo") String tipo);

        @Query("SELECT t.cantidad FROM Transaccion t WHERE t.transaccion_id = :transaccionId")
        Optional<Integer> findCantidadByTransaccionId(@Param("transaccionId") int transaccionId);

        @Query("SELECT t FROM Transaccion t JOIN t.inversionista i JOIN i.usuario u JOIN t.empresa e WHERE t.comisionista.id = :comisionista_id AND t.estado = false AND t.tipo = :tipo")
        List<Transaccion> findVentasDeInversionistas(@Param("comisionista_id") Integer comisionistaId,
